...
import {AutoCompleteProps, DataSourceType} from './autoComplete'
import AutoComplete from "./autoComplete";
//设置了这行属性 会让react-transition-group的动画效果立刻生效，不会再异步触发
...
describe('test AutoComplete component', function () {
    ...
    it('async fetchSuggestions should works fine', async ()=> {
        const testPropsWithPromise:AutoCompleteProps= {
            ...
            //这里要用jest.fn的模拟函数  不然 toHaveBeenCalled监测不到函数被执行
        ...
        const inputNode=wrapper.getByPlaceholderText('auto-complete-3')
        fireEvent.change(inputNode,{target:{value:'a'}})
        await waitFor(()=>{
